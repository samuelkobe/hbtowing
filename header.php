...
<html <?php language_attributes(); ?> class="no-js">
	<head>
		...
		<script src="https://cdn.jsdelivr.net/npm/vue@2.6.12/dist/vue.js" charset="utf-8"></script>
	...
	<body <?php body_class(); ?>>
		...
		<div id="app" class="wrapper bg-gray-900">
			...
			<!-- header -->
			<header :class="[isOpen ? 'bg-white open-menu' : 'bg-transparent', {'scrolled' : !view.atTopOfPage}]" class="md:bg-transparent fixed w-full z-50 top-0" role="banner">
				<div class="container mx-auto md:flex md:justify-start md:items-center">
					...
					<div class="flex items-center justify-between md:mr-auto px-4 py-8">
						...
						<div class="flex">
							<a href="<?php echo home_url(); ?>" class="flex items-start">
								<?php if ( get_field( 'menu_logo', 'option' ) ) : ?>
									<img class="h-12 lg:h-16" src="<?php the_field( 'menu_logo', 'option' ); ?>" alt="<?php bloginfo('name'); ?>" />
								<?php endif ?>
								<div class="flex flex-col items-left pl-6">
									<h1 :class="isOpen ? 'text-tertiary' : 'text-white'" class="max-w-logo lg:max-w-none md:text-white text-base hidden lg:block lg:text-2xl font-bold -my-1 leading-snug lg:my-0 lg:leading-less"><?php bloginfo('name'); ?></h1>
						...
						<div class="block md:hidden">
							<button @click="isOpen = !isOpen" type="button" name="hamburger-menu" :class="isOpen ? 'text-tertiary' : 'text-white'"  class="hamburger text-white focus:outline-none">
					...
					<div :class="isOpen ? 'flex h-screen' : 'hidden h-auto'" class="md:h-auto md:flex justify-end">
						<!-- nav -->
					...
					<div :class="isOpen ? 'block' : 'hidden'" class="px-4 md:block">
						<a href="tel:<?php the_field( 'menu_phone_number', 'option' ); ?>" class="button flex flex-row items-center justify-center text-sm font-bold whitespace-no-wrap">
							<svg class="h-4 w-4 mr-2 fill-current" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
